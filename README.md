...
- 触发前移动超过容差会按普通拖拽处理，并取消本次语音；触发后拖动会被忽略，直到松开鼠标，以避免选择文本或拖动对象。
- 不区分外接鼠标和触控板产生的左键事件。轻点来点按、轻点拖移、拖移锁定等是否等同持续按下由系统设置决定；这个版本不读取设备原始触摸数据。
- 原程序仍会收到起始点击，输入焦点可能因此改变；只有语音已开始后的拖拽事件会被拦截。
...
- `Sources/main.swift`：菜单栏、权限入口、全局监听、语音期间的拖拽拦截和释放处理。
...
- `--enable` 启动参数用于立即请求所需权限并启用；`--diagnostics` 输出本次状态到标准输出，不记录输入文字。正常双击会恢复上次明确选择的开关状态；首次运行默认暂停。0.4 起，休眠、锁屏和会话切换会暂时暂停，全部恢复后自动恢复先前的启用状态；手动暂停仍保持关闭。
