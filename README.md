...
这是验证 当前输入法或语音软件能否接受**软件生成 Fn** 的最小实验，不是语音识别软件。原生 Swift + AppKit/CoreGraphics；无网络请求、无第三方依赖、不录音、不自动开机启动。默认暂停，不会一打开就接管输入或弹出权限请求。

## 直接运行

1. 推荐解压 **MouseVoice.app.zip**，把里面的 **MouseVoice.app** 移到“应用程序”文件夹，再双击打开。菜单栏出现 `MV ○`，没有主窗口或 Dock 图标。本目录也保留直接构建的 app；但本机 Documents 的文件同步服务会反复给 `.app` 添加 Finder 元数据，导致严格签名检查失败，因此优先使用压缩包中的干净副本，并放在非同步的应用程序文件夹。
2. 点击菜单栏 `MV`，按需要使用“申请输入监控权限”和“申请辅助功能权限”。在 **系统设置 → 隐私与安全性 → 输入监控 / 辅助功能** 中允许 **MouseVoice**。如果系统未列出它，使用 `+` 手动添加你实际运行的 app。
3. 如果系统要求退出重开，照做。授权后点击 **启用（开始测试）**，看到 `MV ●`。
4. 确保 当前语音软件 已运行，并且**真实键盘长按 Fn、松开 Fn** 能正常开始和结束语音。把光标放到可输入文字的位置，实际按住鼠标左键约 1.2 秒；或**物理按下触控板并保持**，不要只轻点。
5. 保持基本不动，触发时图标变成 `MV ↑`；松开结束。通过 当前语音软件 的录音界面、声音提示或实际转写判断结果。

工具本身不需要麦克风、屏幕录制或自动化权限。当前语音软件 自己的录音权限由 当前语音软件 管理。监听权限与发送权限分开检查；本机不同版本的 macOS 可能允许已获辅助功能权限的进程直接监听，菜单会显示实际检测值。

## 交互规则

- 原地保持默认 **1.2 秒**后发送 Fn down；松开时发送 Fn up。
...
| 原地按住超过 1.2 秒后松开 | MV ↑；合成 Fn 回读通常增加 2；观察 当前语音软件 是否开始并停止 |
...
3. 点击“重新载入配置”，然后按 当前语音软件 的行为选择以下模式，最后重新启用：
   - **快捷键：按住 / 松开**（`shortcutHold`）：达到 1.2 秒发送快捷键按下，鼠标松开发送快捷键释放，适合按住说话。
   - **快捷键：开始 / 结束各点一次**（`shortcutToggle`）：达到 1.2 秒点按一次快捷键，鼠标松开再点按一次，适合开关式录音。必须先确认 当前语音软件 已停止录音；否则开关状态可能反向。它没有读取 当前语音软件 的录音状态。
...
不需要 Xcode 工程、Swift Package 下载或联网。脚本在临时目录构建 `.app`、使用本地 ad-hoc 签名并严格验证，然后输出 `.app` 和排除 Finder 元数据的 `MouseVoice.app.zip`；没有 Apple Developer 分发签名或公证。重新构建、移动路径后，macOS 可能要求重新授权。建议先放到最终位置再授权。对外分发到其他 Mac 的 Gatekeeper 行为未验证。
...
- `--enable` 启动参数用于立即请求所需权限并启用；`--diagnostics` 输出本次状态到标准输出，不记录输入文字。正常双击仍默认暂停。
